...
const allCategory =document.querySelector(".nav_header-second");



 
let defoltCategory = document.querySelectorAll(".nav_header-second__li");
...
function getСategory(){ 
  ...
  let sumAttempt = 0;
  function getSumAttempt(){
    sumAttempt +=1;
  }

    allCategory.addEventListener('click', (event)=>{
    const liClick = event.target.closest('li');
    let currentCategory =  liClick.getAttribute("value");

    for (let i = 0; i < defoltCategory.length; i++) {
      defoltCategory[i].addEventListener("click", function() {
  ...
  switch(currentCategory){
      case '1':
        var category = Array.from(birdsData[0].sort(() => .5 - Math.random()));
          console.log(category);
          break;
      case '2':
         category = Array.from(birdsData[1].sort(() => .5 - Math.random()));
          console.log(category);

          break;
      case '3':
         category = Array.from(birdsData[2].sort(() => .5 - Math.random()));
          console.log(category);

          break;
      case '4': 
          category = Array.from(birdsData[3].sort(() => .5 - Math.random()));
          console.log(category);

          break;
      case '5':
          category = Array.from(birdsData[4].sort(() => .5 - Math.random()));
          console.log(category);

          break;
      case '6':
          category = Array.from(birdsData[5].sort(() => .5 - Math.random()));
          console.log(category);
...
function addQuestionTrack(){
  const soundTrack = document.querySelector(".task_task-block");
  soundTrack.innerHTML = `
  <div class="task_task-block__name">******</div>
  <audio controls="controls" id="audio_player">
    <source id="${category[1].id}" src="${category[1].audio}" type="audio/ogg"/>
  </audio>
      `;
}
const idTasckAudio = category[1].id;
function addPossibleAnswer(){
  
  const answer = document.querySelector(".possible-answer_ul");
  category.sort(() => .5 - Math.random());
  
  for(let i = 0; i < category.length; i++){
    answer.innerHTML = `
    <div class="lock_ul"></div>
    <li class="possible-answer_ul__wrapper">
    <input type="radio" class="possible-answer_ul__li" id="${category[0].id}" >
    <div for="name" class="possible-answer_ul__name">${category[0].name}
    </div>
    </li>
    <div class="lock_ul"></div>
    <li class="possible-answer_ul__wrapper">
    <input type="radio" class="possible-answer_ul__li" id="${category[1].id}" >
    <label for="name" class="possible-answer_ul__name">${category[1].name}
    </label>
    </li>
    <div class="lock_ul"></div>
    <li class="possible-answer_ul__wrapper">
    <input type="radio" class="possible-answer_ul__li" id="${category[2].id}" >
    <label for="name" class="possible-answer_ul__name">${category[2].name}
    </label>
    </li>
    <div class="lock_ul"></div>
    <li class="possible-answer_ul__wrapper">
    <input type="radio" class="possible-answer_ul__li" id="${category[3].id}" >
    <label for="name" class="possible-answer_ul__name">${category[3].name}
    </label>
    </li>
    <div class="lock_ul"></div>
    <li class="possible-answer_ul__wrapper">
    <input type="radio" class="possible-answer_ul__li" id="${category[4].id}" >
    <label for="name" class="possible-answer_ul__name">${category[4].name}
    </label>
    </li>
    <div class="lock_ul"></div>
    <li class="possible-answer_ul__wrapper">
    <input type="radio" class="possible-answer_ul__li" id="${category[5].id}"  >
    <label for="name" class="possible-answer_ul__name">${category[5].name}
    </label>
    </li>
    `;
  }
...
function selectAnswer(){
  ...
  const winBlock1 = document.querySelector(".task");
  const winBlock2 = document.querySelector(".correct-answer");
  const unSelectUl = Array.from(document.querySelectorAll(".lock_ul"));
  const selectWin = document.querySelector(".lock");
  const select = document.querySelector(".possible-answer_ul");
  const selectButton = Array.from(document.querySelectorAll(".possible-answer_ul__li"));
  const selectNameArea = Array.from(document.querySelectorAll(".possible-answer_ul__wrapper"));
    ...
    for (let i = 0; i < selectButton.length; i++) {
      selectButton[i].addEventListener("click", function() {
        ...
        console.log(sumAttempt);
        const idNameAnswer =  category[i].id;
        if(idTasckAudio === idNameAnswer){
            selectButton[i].classList.add("checked_corect");
            selectWin.style.display = "grid";
            winBlock1.innerHTML = `
            <div class="task_img">
            <img class="task_img__img" src="${category[i].image}" alt="bird">
          </div>
              <div class="task_task-block">
                <div class="task_task-block__name">${category[i].name}</div>
                <audio controls="controls" id="audio_player">
                  <source id="${category[i].id}" src="${category[i].audio}" type="audio/ogg" />
                </audio>
              </div>
                `;

                winBlock2.innerHTML = `
            <div class="wrapper_correct-answer">
              <div class="wrapper_correct-answer__img">
                <img class="wrapper_correct-answer__img__img" src="${category[i].image}" alt="bird">
              </div>
                <h3 class="wrapper_correct-answer__bird__ru">
                ${category[i].name}
                </h3>
                <h4 class="wrapper_correct-answer__bird__en">
                ${category[i].species}species
                </h4>
                <audio class="wrapper_correct-answer__bird__audio" controls="controls" id="audio_player">
                  <source id="${category[i].id}" src="${category[i].audio}" type="audio/ogg" />
                </audio>
                <p class="wrapper_correct-answer__bird__p">
        ...
        }else if(idTasckAudio !== idNameAnswer){
          selectButton[i].classList.add("checked_incorect");
          unSelectUl[i].style.display = "grid";
        }
      });
    }
    for (let i = 0; i < selectNameArea.length; i++) {
      selectNameArea[i].addEventListener("click", function() {
      ...
      if(idTasckAudio === idNameAnswer){
         ...
         <img class="task_img__img" src="${category[i].image}" alt="bird">
           ...
           <div class="task_task-block">
             <div class="task_task-block__name">${category[i].name}</div>
             <audio class="task_task-block__audio" controls="controls" id="audio_player">
               <source id="${category[i].id}" src="${category[i].audio}" type="audio/ogg" />
             </audio>
           </div>
             `;
